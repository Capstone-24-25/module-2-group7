...
1. **Binary Classification:** The trained binary model predicts probabilities for each test sequence. The predictions are **thresholded at 0.5** to decide class labels, meaning that the probability greater than 0.5 is considered as relevant content. The **accuracy** is about **0.715**, with a sensitivity around **0.787** and specificity around **0.658**. This indicates that the model correctly identifies **78.7%** website that is relevant and correctly identifies **65.8%** websites as not relevant. 


2. **Multiclass Classification:** For multi-class classification, the accuracy is about **0.638**, with sensitivity and specificity for each category listed below.
